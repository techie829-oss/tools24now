...
interface POData {
    ...
    // Shipping
    shipTo: string; // Different shipping address than billing
    expectedDate: string;
...
export default function PurchaseOrderGenerator() {
    ...
    const [items, setItems] = useState<LineItem[]>([
        { id: '1', description: 'MacBook Pro M3 Max (16-inch, 32GB)', hsn: '847130', sku: 'APL-MBP-M3', quantity: 2, rate: 249000, amount: 498000 },
        { id: '2', description: 'Dell UltraSharp 27" 4K Monitor', hsn: '852852', sku: 'DEL-U2723QE', quantity: 2, rate: 54000, amount: 108000 },
    ]);
    ...
    const [data, setData] = useState<POData>({
        ...
        buyer: 'TechSolutions India Pvt Ltd.\nUnit 402, Cyber Park\nGurugram, Haryana 122002',
        vendor: 'Imagine Store (Auth Reseller)\nDLF Cyber Hub\nGurugram, Haryana',
        currency: '₹',
        taxType: 'gst_intra',
        taxRate: 18,
        taxInclusive: true, // POs often quote inclusive price from vendors
        discount: 0,
        notes: 'Please reference PO number on all invoices.\nDeliver between 10 AM - 6 PM on weekdays.',

        shipTo: 'TechSolutions India Pvt Ltd.\nUnit 402, Cyber Park\nGurugram, Haryana 122002',
    ...
    const downloadPDF = async () => {
        ...
        try {
            const element = document.getElementById('po-preview');
            if (!element) return;
            ...
            <div className="max-w-[1800px] mx-auto">
                <div className="flex justify-between items-center mb-6">
                    <div>
                        ...
                        <p className="text-sm text-gray-500 mt-1">Create professional procurement orders</p>
                    </div>
                    <button
                        onClick={downloadPDF}
                        disabled={isGenerating}
                        className="inline-flex items-center px-5 py-2.5 border border-transparent text-sm font-medium rounded-lg shadow-sm text-white bg-orange-600 hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 disabled:opacity-50 transition-all"
                    >
                        {isGenerating ? 'Generating...' : <><Download className="w-4 h-4 mr-2" /> Download PO</>}
                    </button>
                </div>
                ...
                <div className="flex flex-col xl:flex-row gap-6 items-start">
                    ...
                    <div className="w-full xl:w-1/2 space-y-5">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select
                                        ...
                                        className="w-full rounded border-gray-300 shadow-sm focus:border-orange-500 focus:ring-orange-500 text-sm py-1.5 text-gray-900"
                                    >
                                ...
                                <div>
                                    ...
                                    <label className="flex items-center gap-2 mt-2 cursor-pointer">
                                        <input type="checkbox" checked={data.taxInclusive} onChange={e => setData({ ...data, taxInclusive: e.target.checked })} className="rounded text-orange-600 focus:ring-orange-500" />
                        ...
                        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-5 grid grid-cols-2 gap-6">
                            <div>
                                <label className="block text-xs font-semibold text-gray-500 uppercase mb-1">PO #</label>
                                <input type="text" value={data.number} onChange={e => setData({ ...data, number: e.target.value })} className="w-full rounded border-gray-300 font-mono text-sm py-1.5 text-gray-900" />
                            </div>
                            <div className="grid grid-cols-2 gap-2">
                                ...
                                <div>
                                    ...
                                    <input type="date" value={data.dueDate} onChange={e => setData({ ...data, dueDate: e.target.value })} className="w-full rounded border-gray-300 text-sm py-1.5 text-gray-900" />
                                </div>
                            </div>
                            <div className="col-span-1">
                                <label className="block text-xs font-semibold text-gray-500 uppercase mb-1">Buyer (You)</label>
                                <textarea rows={3} value={data.buyer} onChange={e => setData({ ...data, buyer: e.target.value })} className="w-full rounded border-gray-300 text-sm p-2 text-gray-900" />
                            </div>
                            <div className="col-span-1">
                                <label className="block text-xs font-semibold text-gray-500 uppercase mb-1">Vendor (Seller)</label>
                                <textarea rows={3} value={data.vendor} onChange={e => setData({ ...data, vendor: e.target.value })} className="w-full rounded border-gray-300 text-sm p-2 text-gray-900" />
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-5">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="md:col-span-2">
                                    <label className="block text-xs font-semibold text-gray-500 uppercase mb-1">Ship To Address</label>
                                    <textarea rows={2} value={data.shipTo} onChange={e => setData({ ...data, shipTo: e.target.value })} className="w-full rounded border-gray-300 text-sm p-2 text-gray-900" />
                                </div>
                                <div>
                                    <label className="block text-xs font-semibold text-gray-500 uppercase mb-1">Expected Delivery</label>
                                    <input type="date" value={data.expectedDate} onChange={e => setData({ ...data, expectedDate: e.target.value })} className="w-full rounded border-gray-300 text-sm py-1.5 text-gray-900" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-5">
                            ...
                            <button onClick={addItem} className="mt-3 w-full py-2 border-2 border-dashed border-gray-300 rounded text-sm text-gray-600 hover:border-orange-500 hover:text-orange-600 font-medium">Add Item</button>
                        </div>
                    ...
                    <div className="w-full xl:w-1/2 min-h-[calc(100vh-100px)] flex justify-center items-start bg-gray-500/10 rounded-xl border-2 border-dashed border-gray-300 overflow-hidden shadow-inner p-4">
                        ...
                        >
                            {/* HEADER */}
                            <div className="flex justify-between items-start mb-8 border-b-4 border-orange-600 pb-6">
                                <div>
                                    <h1 className="text-4xl font-extrabold text-[#111827] uppercase tracking-wide">Purchase Order</h1>
                                    <p className="text-orange-600 font-bold mt-1 text-lg">#{data.number}</p>
                                </div>
                            ...
                            <div className="flex justify-between mb-8">
                                <div className="w-5/12">
                                    <h3 className="text-xs font-bold text-[#9ca3af] uppercase tracking-wider mb-2">Vendor (Seller)</h3>
                                    <div className="whitespace-pre-line text-sm leading-relaxed font-medium">{data.vendor}</div>
                                </div>
                                <div className="w-5/12 text-right">
                                    <h3 className="text-xs font-bold text-[#9ca3af] uppercase tracking-wider mb-2">Buyer (Billing)</h3>
                                    <div className="whitespace-pre-line text-sm leading-relaxed font-medium">{data.buyer}</div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-8 mb-8 bg-gray-50 p-4 rounded-lg">
                                <div>
                                    <h3 className="text-xs font-bold text-[#9ca3af] uppercase tracking-wider mb-1">Ship To</h3>
                                    <div className="text-sm text-[#111827] font-medium whitespace-pre-line">{data.shipTo}</div>
                                </div>
                                <div className="text-right">
                                    <h3 className="text-xs font-bold text-[#9ca3af] uppercase tracking-wider mb-1">Expected Delivery</h3>
                                    <div className="text-sm text-[#111827] font-medium">{data.expectedDate}</div>
                                </div>
                            </div>
                            ...
                            <div className="mt-auto">
                                <div className="border-t border-gray-100 pt-6">
                                    <h4 className="text-xs font-bold text-[#9ca3af] uppercase tracking-wider mb-2">Instructions / Notes</h4>
